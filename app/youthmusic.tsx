...
const imgUrl = 'https://i.imgur.com/QSMdzcf.png';
...
export default function App() {
    ...
    const grow = () => {
        if(firstCount == secondCount && secondCount == thirdCount)
            router.navigate('resultmusician');
        else if (secondCount == 0 || firstCount == 0)
            router.navigate('resultwanderer');
        else if (thirdCount > firstCount && thirdCount > secondCount)
            router.navigate('resultwanderer');
        else router.navigate('resultmusician');
    };

    return (
        <View style = {styles.container}>
            <View style = {{ height: 100}}>
                <Text style = {{ textAlign: 'center', fontSize: 40, fontWeight: 'bold' }}>음악을 즐기는 청년</Text>
            </View>
