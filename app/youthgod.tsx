...
export default function App() {
    ...
    const grow = () => {
        if(firstCount == secondCount && secondCount == thirdCount)
            router.navigate('resultpope');
        else if (secondCount == 0 || firstCount == 0)
            router.navigate('resultcult');
        else if (thirdCount > firstCount && thirdCount > secondCount)
            router.navigate('resultcult');
        else router.navigate('resultpope');
    };

    return (
        <View style = {styles.container}>
            <View style = {{ height: 100}}>
                <Text style = {{ textAlign: 'center', fontSize: 40, fontWeight: 'bold' }}>신학을 즐기는 청년</Text>
            </View>
