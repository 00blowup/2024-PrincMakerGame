...
const imgUrl = 'https://i.imgur.com/2AgECXV.png';
...
export default function App() {
    ...
    const grow = () => {
        if(firstCount == secondCount && secondCount == thirdCount)
            router.navigate('resultplay');
        else if (secondCount == 0 || firstCount == 0)
            router.navigate('resultforbidden');
        else if (thirdCount > firstCount && thirdCount > secondCount)
            router.navigate('resultforbidden');
        else router.navigate('resultplay');
    };

    return (
        <View style = {styles.container}>
            <View style = {{ height: 100}}>
                <Text style = {{ textAlign: 'center', fontSize: 40, fontWeight: 'bold' }}>문학을 즐기는 청년</Text>
            </View>
