...
export default function App() {
    ...
    const grow = () => {
        if(firstCount == secondCount && secondCount == thirdCount)
            router.navigate('resultking');
        else if (secondCount == 0 || firstCount == 0)
            router.navigate('resulttyrant');
        else if (thirdCount > firstCount && thirdCount > secondCount)
            router.navigate('resulttyrant');
        else router.navigate('resultking');
    };

    return (
        <View style = {styles.container}>
            <View style = {{ height: 100}}>
                <Text style = {{ textAlign: 'center', fontSize: 40, fontWeight: 'bold' }}>제왕학을 즐기는 청년</Text>
            </View>
