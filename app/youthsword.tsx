...
export default function App() {
    ...
    const grow = () => {
        if(firstCount == secondCount && secondCount == thirdCount)
            router.navigate('resultknight');
        else if (secondCount == 0 || firstCount == 0)
            router.navigate('resultslayer');
        else if (thirdCount > firstCount && thirdCount > secondCount)
            router.navigate('resultslayer');
        else router.navigate('resultknight');
    };

    return (
        <View style = {styles.container}>
            <View style = {{ height: 100}}>
                <Text style = {{ textAlign: 'center', fontSize: 40, fontWeight: 'bold' }}>무술을 즐기는 청년</Text>
            </View>
